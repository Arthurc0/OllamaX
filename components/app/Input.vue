<template>
    <div class="flex flex-col gap-2">
        ...
        <select v-if="type === InputTypeEnum.SELECT" @change="updateValue" :id="field.name">
            ...
            <option v-else v-for="option in selectOptions" :key="option.value" :value="option.value" :selected="!!option.selected && !pending">{{ option.label }}</option>
        </select>
        ...
        <div v-else-if="[InputTypeEnum.TEXT, InputTypeEnum.PASSWORD].includes(type)">
            <input v-if="style === InputStyleEnum.EDITABLE" autocomplete="off" class="w-full p-1 text-sm transition duration-200 hocus:bg-light-gray" :class="[!!errors[field.name] ? 'border-2 border-red' : '']" @input="updateValue" :type="type" :id="field.name" />

            <input v-else autocomplete="off" class="w-full p-3 text-sm bg-slate-200/80 rounded-md border-2 transition duration-200 focus:bg-slate-200/50" :class="[!!errors[field.name] ? 'border-2 border-red' : 'border-slate-300/80']" @input="updateValue" :type="type" :id="field.name" />
        </div>
...
const setFormValue = inject('setFormValue') as (names: string[], event: Event, validateField: boolean) => void;

const updateValue = (e: Event): void => {
    if (props.linkedFields?.length) setFormValue([props.field.name, ...props.linkedFields.map((f) => f.name)], e, props.field?.validate ?? true);
    else setFormValue([props.field.name], e, props.field?.validate ?? true);
};
</script>
