<template>
    <div class="flex flex-col items-start w-full h-full gap-5">
        <div tabindex="1" role="tablist" class="select-none flex items-center gap-1 bg-gray/5 rounded-lg p-[5px]" @keyup="keyUp">
            <span class="cursor-pointer font-semibold text-sm rounded-lg py-[6px] px-[11px] transition duration-200 hover:text-gray/80" :class="[tab.title === selectedTab ? 'text-gray bg-white shadow-lg' : 'text-gray/40']" v-for="tab in childrenProps" :key="tab.title" @click="selectTab(tab.title)">{{ tab.title }}</span>
        </div>
